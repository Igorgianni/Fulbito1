...
function App() {
  ...
  React.useEffect(() => {
    ...
    if (jugadoresGuardados) {
      ...
      // Si no hay jugadores en localStorage, cargar desde el archivo JSON
  ...
  const generarEquipos = () => {
    const jugadoresOrdenados = [...jugadores].sort((a, b) => {
      const promedioA = Object.values(a).filter(val => typeof val === 'number').reduce((sum, val) => sum + val, 0) / 6;
      const promedioB = Object.values(b).filter(val => typeof val === 'number').reduce((sum, val) => sum + val, 0) / 6;
